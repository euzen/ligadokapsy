...
export default function HomeScreen() {
  ...
  return (
    <ScrollView className="flex-1 bg-canvas" contentContainerClassName="items-center pb-24">
      <View className="w-full max-w-6xl px-5 py-10 md:py-20">
        ...
        <View className="mt-16">
          <Text className="text-3xl font-black text-ink">{t('home.competitionsTitle')}</Text>
          <Text className="mt-2 text-muted">{t('home.competitionsSubtitle')}</Text>
          {loading ? (
            ...
            <View className="mt-6 flex-row flex-wrap gap-4">
              {previewTournaments.map((tournament) => (
                <Pressable
                  ...
                  className="min-w-72 flex-1 rounded-2xl border border-slate-200 bg-white p-5 active:bg-slate-50"
                >
                  <View className="flex-row items-center justify-between gap-3">
                    ...
                    <View className={`rounded-full px-3 py-1 ${tournament.status === 'published' ? 'bg-emerald-100' : 'bg-slate-200'}`}>
                      <Text className={`text-xs font-black uppercase ${tournament.status === 'published' ? 'text-emerald-700' : 'text-slate-600'}`}>
              ...
              {!previewTournaments.length && (
                <Text className="flex-1 rounded-2xl border border-dashed border-slate-300 bg-slate-50 py-8 text-center text-muted">
        ...
        <View className="mt-16">
          <Text className="text-3xl font-black text-ink">{t('common.appName')}</Text>
          <Text className="mt-2 text-muted">{t('home.description')}</Text>
          <View className="mt-6 flex-row flex-wrap gap-4">
            {featureOrder.map((key, index) => (
              <View key={key} className="min-w-72 flex-1 rounded-2xl border border-slate-200 bg-white p-6">
                <Text className="font-mono text-sm font-black text-brand">0{index + 1}</Text>
                <Text className="mt-4 text-xl font-black text-ink">{t(`home.features.${key}`)}</Text>
                <Text className="mt-2 text-sm leading-6 text-muted">{t(`home.features.${key}Desc`)}</Text>
              </View>
            ))}
          </View>
        </View>
