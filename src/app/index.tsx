...
function StatItem({ value, label, loading }: { value: number; label: string; loading: boolean }) {
  return (
    <View className="min-w-40 flex-1 rounded-2xl border border-slate-200 bg-white p-5 dark:border-slate-700 dark:bg-slate-800/60">
      <Text className="text-center text-4xl font-black text-emerald-600 dark:text-emerald-400">{loading ? '–' : value}</Text>
      <Text className="mt-2 text-center text-xs font-black uppercase tracking-wider text-slate-600 dark:text-slate-300">{label}</Text>
    </View>
  );
}
...
export default function HomeScreen() {
  ...
  return (
    <ScrollView className="flex-1 bg-slate-50 dark:bg-slate-900" contentContainerClassName="items-center pb-24">
      <View className="w-full max-w-6xl px-5 py-10 md:py-20">
        ...
        <View className="mt-16">
          <Text className="text-3xl font-black text-slate-900 dark:text-white">{t('home.competitionsTitle')}</Text>
          <Text className="mt-2 text-slate-500 dark:text-slate-400">{t('home.competitionsSubtitle')}</Text>
          {loading ? (
            ...
            <View className="mt-6 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 items-stretch">
              {previewTournaments.map((tournament) => (
                <Pressable
                  ...
                  className="flex h-full flex-col justify-between rounded-2xl border border-slate-200 bg-white p-5 active:bg-slate-100 dark:border-slate-700 dark:bg-slate-800 dark:active:bg-slate-700"
                >
                  <View className="flex-row items-start justify-between gap-3">
                    <View className="flex-1">
                      <View className="flex-row flex-wrap items-center gap-2">
                        <Text className="text-xl font-black text-slate-900 dark:text-white">{tournament.name}</Text>
                    ...
                    <View className={`rounded-full px-3 py-1 ${tournament.status === 'published' ? 'bg-emerald-100 dark:bg-emerald-500/20' : 'bg-slate-100 dark:bg-slate-700'}`}>
                      <Text className={`text-xs font-black uppercase ${tournament.status === 'published' ? 'text-emerald-700 dark:text-emerald-400' : 'text-slate-600 dark:text-slate-300'}`}>
              ...
              {!previewTournaments.length && (
                <Text className="col-span-full rounded-2xl border border-dashed border-slate-300 bg-slate-100 py-8 text-center text-slate-500 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-400">
        ...
        <View className="mt-16">
          <Text className="text-3xl font-black text-slate-900 dark:text-white">{t('common.appName')}</Text>
          <Text className="mt-2 text-slate-500 dark:text-slate-400">{t('home.description')}</Text>
          <View className="mt-6 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 items-stretch">
            {featureOrder.map((key, index) => (
              <View key={key} className="flex h-full flex-col justify-between rounded-2xl border border-slate-200 bg-white p-6 dark:border-slate-700 dark:bg-slate-800">
                <View>
                  <Text className="font-mono text-sm font-black text-brand">0{index + 1}</Text>
                  <Text className="mt-4 text-xl font-black text-slate-900 dark:text-white">{t(`home.features.${key}`)}</Text>
                  <Text className="mt-2 text-sm leading-6 text-slate-500 dark:text-slate-400">{t(`home.features.${key}Desc`)}</Text>
                </View>
              </View>
            ))}
          </View>
