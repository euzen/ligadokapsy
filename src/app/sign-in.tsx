...
export default function SignInScreen() {
  ...
  const submit = async () => { setError(''); setLoading(true); try { await signIn(email, password); toast.success(t('toast.signedIn')); } catch (reason) { const message = t(reason instanceof Error ? reason.message : 'auth.invalidCredentials'); setError(message); toast.error(t('toast.signInFailed'), message); } finally { setLoading(false); } };
