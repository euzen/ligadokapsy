...
export function RosterPlayerModal({ player, onClose, onSave }: { player?: RosterPlayer; onClose: () => void; onSave: (values: Values) => Promise<void> }) {
  ...
  const [name, setName] = useState(player?.player_name ?? ''); const [number, setNumber] = useState(player?.jersey_number?.toString() ?? ''); const [position, setPosition] = useState(player?.position ?? ''); const [captain, setCaptain] = useState(player?.is_captain ?? false); const [saving, setSaving] = useState(false);
