...
export async function listTournamentTeams(tournamentId: string) { if (sb.isSupabaseMode) return sb.sbTournamentTeams(tournamentId); try { return await request<Team[]>(`/tournaments/${tournamentId}/teams`); } catch (error) { if ((error as Error).message === 'tournaments.notFound') return []; throw error; } }
...
export async function listTournamentTeamAssignments(tournamentId: string) { if (sb.isSupabaseMode) return sb.sbTournamentTeamAssignments(tournamentId); try { return await request<TournamentTeam[]>(`/tournaments/${tournamentId}/teams-assignments`); } catch (error) { if ((error as Error).message === 'tournaments.notFound') return []; throw error; } }
