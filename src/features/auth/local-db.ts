...
export async function createTeamRoster(teamId: string, values: Omit<RosterPlayer, 'id' | 'team_id' | 'user_id'>, _actor: UserProfile) { if (sb.isSupabaseMode) { const result = await sb.sbCreateTeamRoster(teamId, values); dataListeners.forEach((listener) => listener()); return result; } const result = await request<RosterPlayer>(`/teams/${teamId}/rosters`, { method: 'POST', body: JSON.stringify(values) }); dataListeners.forEach((listener) => listener()); return result; }
