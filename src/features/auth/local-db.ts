...
export async function listTournamentTeams(tournamentId: string) { if (sb.isSupabaseMode) return sb.sbTournamentTeams(tournamentId); try { return await request<Team[]>(`/tournaments/${tournamentId}/teams`); } catch (error) { if ((error as Error).message.includes('notFound')) return []; throw error; } }
...
export async function createMatch(values: Omit<Match, 'id' | 'status' | 'home_score' | 'away_score' | 'clock_seconds' | 'clock_started_at' | 'current_period'>, _actor: UserProfile) { if (sb.isSupabaseMode) { const result = await sb.sbCreateMatch(values); dataListeners.forEach((listener) => listener()); return result; } const result = await request<Match>('/matches', { method: 'POST', body: JSON.stringify(values) }); dataListeners.forEach((listener) => listener()); return result; }
...
export async function listTournamentTeamAssignments(tournamentId: string) { if (sb.isSupabaseMode) return sb.sbTournamentTeamAssignments(tournamentId); try { return await request<TournamentTeam[]>(`/tournaments/${tournamentId}/teams-assignments`); } catch (error) { if ((error as Error).message.includes('notFound')) return []; throw error; } }
...
export async function linkTeamRosterPlayer(teamRosterId: string, userId: string | null, _actor: UserProfile) { if (sb.isSupabaseMode) { await sb.sbLinkTeamRosterPlayer(teamRosterId, userId); dataListeners.forEach((listener) => listener()); return; } await request(`/team-rosters/${teamRosterId}/link`, { method: 'PATCH', body: JSON.stringify({ userId }) }); dataListeners.forEach((listener) => listener()); }
