...
import type { AdminMetrics, AppRole, EditableProfile, Match, MatchAccess, MatchEvent, PublicMatch, Sport, Team, Tournament, TournamentStatus, UserProfile } from '@/types/database';
...
export async function createTournament(values: Omit<Tournament, 'id' | 'created_by' | 'status'>, _actor: UserProfile) { if (sb.isSupabaseMode) { const result = await sb.sbCreateTournament(values); dataListeners.forEach((listener) => listener()); return result; } const result = await request<Tournament>('/tournaments', { method: 'POST', body: JSON.stringify(values) }); dataListeners.forEach((listener) => listener()); return result; }
export async function updateTournament(id: string, values: Partial<Omit<Tournament, 'id' | 'created_by'>>, _actor: UserProfile) { if (sb.isSupabaseMode) { const result = await sb.sbUpdateTournament(id, values); dataListeners.forEach((listener) => listener()); return result; } const result = await request<Tournament>(`/tournaments/${id}`, { method: 'PATCH', body: JSON.stringify(values) }); dataListeners.forEach((listener) => listener()); return result; }
...
export async function recordMatchEvent(secret: string, event: Pick<MatchEvent, 'event_type' | 'team_id' | 'player_name'>) { if (sb.isSupabaseMode) { await sb.sbRecordEvent(secret, event); dataListeners.forEach((listener) => listener()); return; } await request('/scorekeeper/event', { method: 'POST', body: JSON.stringify({ secret, ...event }) }); dataListeners.forEach((listener) => listener()); }
