...
function mapProfile(row: any): UserProfile { return { id: row.id, first_name: row.first_name, last_name: row.last_name, email: row.email, role: row.role, favoriteSport: row.favorite_sport, avatarUrl: row.avatar_url, profileColor: row.profile_color, createdAt: row.created_at, language: row.language }; }
...
export async function sbLinkTeamRosterPlayer(teamRosterId: string, userId: string | null) { const { error } = await supabase.from('team_rosters').update({ user_id: userId }).eq('id', teamRosterId); fail(error); }
